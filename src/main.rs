...
fn detect_absolute(ground: &Grid, perceived: &mut Grid, &(x, y): &(usize, usize), kernel: ndarray::Array2<bool>) {
...
fn detect_uncertain(ground: &Grid, perceived_probs: &mut Array2<f64>, &(x, y): &(usize, usize), kernel: ndarray::Array2<f64>) {
    assert!(kernel.ncols() % 2 == 1, "Kernel has no middle column");
    assert!(kernel.nrows() % 2 == 1, "Kernel has no middle row");
    for i in 0..kernel.nrows() {
        for j in 0..kernel.ncols() {
        ...
        for m_row in 0..matrix.nrows() {
            for k_col in 0..kernel.ncols() {
                for k_row in 0..kernel.nrows() {
                    if m_row + k_row >= kernel.nrows()/2
                        && m_col + k_col >= kernel.ncols()/2 {
...
fn prob_as_heatmap(probs: &Array2<f64>,
     drawing_area: &DrawingArea<BitMapBackend, Shift>,)
      -> Result<(), Box<dyn std::error::Error>> {
        ...
        for y in 0..probs.nrows() {
            for x in 0..probs.ncols() {
                ...
                match sub_areas[idx].fill(&lerp_hsv(LOW, HIGH, prob)) {
                    Ok(()) => {},
                    _ => {println!("Failed to fill with colour")}
                }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // panic!();
    let mut probs = blur_to_prob(&grid);
    ...
    while !success(&current) {
        ...
        detect_absolute(&grid, &mut guess, &current, ndarray::Array::from_elem((11,11), true));
