...
fn plot_grid(
    ...
    for (idx, sub_area) in (0..).zip(sub_areas.iter()) {
        if grid.has_vertex((idx % grid.width, idx / grid.width)) {
            sub_area.fill(&RGBColor(red, 100, 100))?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use rand::Rng;
    let grid = random_grid(1000, 1000, 131);
    ...
    let successors_weighted = |p: &(usize, usize)| grid.neighbours(*p).iter().map(|&p| (p, 20*p.0)).collect::<Vec<_>>();
    ...
    let result_astar:Option<(Vec<(usize, usize)>, usize)>  = astar(
                        &start,
                         successors_weighted,
                         |p| grid.distance(*p, goal),
                          |p| *p == goal
                        );
    match result_astar {
        Some((path, _cost)) => save_result(Some(path), grid.clone(), start, goal, "astar.png")?,
        None => save_result(None, grid.clone(), start, goal, "astar.png")?
    }
                
    let result_bfs: Option<Vec<(usize, usize)>> = bfs(start, &mut successors, &mut success);
    // let result_dfs = dfs(start, &mut successors, &mut success);
    ...
    // match result_dfs {
    //     Some(path) => save_result(Some(path), grid.clone(), start, goal, "dfs.png")?,
    //     None => save_result(None, grid.clone(), start, goal, "dfs.png")?
    // }
...
fn save_result(
    ...
    let root = root
    .titled("Path Finding Grid", ("sans-serif", 60))?
